...
class FirebaseController {
  ...
  static Future<List<PhotoLike>> checkForLike(
      String email, String memoId) async {
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection(Constant.PHOTOMEMO_LIKE)
        .where(PhotoLike.PHOTOMEMO_ID, isEqualTo: memoId)
        .where(PhotoLike.LIKED_BY, isEqualTo: email)
        .get();

    var result = <PhotoLike>[];
    querySnapshot.docs.forEach(
      (doc) {
        result.add(PhotoLike.deserialize(doc.data(), doc.id));
      },
    );
    return result;
  }
  ...
  static Future<List<PhotoLike>> getPhotoLikes(String memoId) async {
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection(Constant.PHOTOMEMO_LIKE)
        .where(PhotoLike.PHOTOMEMO_ID, isEqualTo: memoId)
        .get();
  ...
  static Future<List<PhotoComment>> getRecentComment(String memoId) async {
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection(Constant.PHOTOMEMO_COMMENT)
  ...
  static Future<List<Profile>> searchUsers(
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection(Constant.PROFILE)
        .where(Profile.USER_EMAIL, whereIn: searchLabels)
        .get();
    var results = <Profile>[];
    querySnapshot.docs
